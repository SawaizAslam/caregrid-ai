...
> **Status: aspirational reference, not the implemented system.** The
> shipped CareGrid AI backend is the FastAPI service in `backend/` plus
> the Hugging Face Spaces Docker image. This document is a future-work
> blueprint for porting the same logic to a Databricks-native stack
> (Delta + Mosaic Vector Search + MLflow). Treat it as architecture
> reference material rather than a build guide for the current repo.

> **How to use this guide:** open this file alongside the running
> backend. Each section maps to a separate Databricks notebook. Code
> cells are marked with `# CELL N` headers; paste them sequentially
> into notebook cells. Run notebooks in the order 01 -> 02 -> 03 -> 04
> -> 05 -> 06 -> 07 -> 08.
